      ...
      describe("/services/{service}", function()
        ...
        describe("PATCH", function()
          ...
          it_content_types("updates if found by name", function(content_type)
            return function()
              ...
              local in_db = assert(db.services:select_by_name(service.name), { null = ngx.null })
              assert.same(json, in_db)
            end
          end)
        ...
        describe("DELETE", function()
          it("deletes a service", function()
            ...
            local in_db, err = db.services:select({ id = service.id }, { null = ngx.null })
            assert.is_nil(err)
            assert.is_nil(in_db)
          end)
          ...
          it("deletes a service by name", function()
            ...
            local in_db, err = db.services:select_by_name(service.name, { null = ngx.null })
            assert.is_nil(err)
            assert.is_nil(in_db)
          end)
