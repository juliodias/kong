  ...
  describe("Admin API #" .. strategy, function()
    ...
    describe("/routes", function()
      ...
      describe("/routes/{route}", function()
        ...
        describe("PUT", function()
          it_content_types("creates if not found", function(content_type)
            return function()
              ...
              local in_db = assert(db.routes:select({ id = route.id }, { null = ngx.null }))
        ...
        describe("DELETE", function()
          it("deletes a route", function()
            ...
            local in_db, err = db.routes:select({id = route.id}, { null = ngx.null })
            assert.is_nil(err)
            assert.is_nil(in_db)
          end)
      ...
      describe("/routes/{route}/service", function()
        ...
        before_each(function()
          service = bp.services:insert({ host = "example.com", path = "/" }, { null = ngx.null })
          route   = bp.routes:insert({ paths = { "/my-route" }, service = service }, { null = ngx.null })
        end)
        ...
        describe("PATCH", function()
          it_content_types("updates if found", function(content_type)
            return function()
              ...
              local in_db = assert(db.services:select({ id = service.id }, { null = ngx.null }))
              assert.same(json, in_db)
            end
          end)
